...
class MyAgent:
    def __init__(self, id=None, role=None, backstory=None, goal=None, temperature=None, allow_delegation=False, verbose=False, cache= None, llm_provider_model=None, max_iter=None, created_at=None, tools=None, knowledge_source_ids=None):
        ...
        self.role = role or t('agent.default_role')
        self.backstory = backstory or t('agent.default_backstory')
        self.goal = goal or t('agent.default_goal')
        ...
        if self.edit:
            with st.expander(t('agent.title', role=self.role), expanded=True):
                with st.form(key=form_key):
                    ...
                    self.llm_provider_model = st.selectbox(t('agent.llm_provider'), options=llm_providers_and_models(), index=llm_providers_and_models().index(self.llm_provider_model))
            ...
            with st.expander(expander_title, expanded=False):
                st.markdown(f"**{t('agent.role')}:** {self.role}")
                st.markdown(f"**{t('agent.backstory')}:** {self.backstory}")
                st.markdown(f"**{t('agent.goal')}:** {self.goal}")
                st.markdown(f"**{t('agent.allow_delegation')}:** {self.allow_delegation}")
                st.markdown(f"**{t('agent.verbose')}:** {self.verbose}")
                st.markdown(f"**{t('agent.cache')}:** {self.cache}")
                st.markdown(f"**{t('agent.llm_provider')}:** {self.llm_provider_model}")
                st.markdown(f"**{t('agent.temperature')}:** {self.temperature}")
                st.markdown(f"**{t('agent.max_iterations')}:** {self.max_iter}")
                st.markdown(f"**{t('agent.tools')}:** {[self.get_tool_display_name(tool) for tool in self.tools]}")
                ...
                if self.knowledge_source_ids and 'knowledge_sources' in ss:
                    ...
                    if knowledge_sources:
                        st.markdown(f"**{t('agent.knowledge_sources')}:**")
                ...
                with col1:
                    ...
                    st.button(t('button.edit'), on_click=self.set_editable, args=(True,), key=btn_key)
                with col2:
                    del_key = f"del_btn_{rnd_id()}"
                    st.button(t('button.delete'), on_click=self.delete, key=del_key)
