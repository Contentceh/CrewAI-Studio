...
class PageCrewRun:
    ...
    def draw_placeholders(self, crew):
        ...
        if placeholders:
            st.write(t('crew_run.placeholders'))
    ...
    def control_buttons(self, selected_crew):
        ...
        if st.button(t('crew_run.stop_button'), disabled=not ss.running):
    ...
    def display_result(self):
        ...
        with console_container.container():
            with st.expander(t("crew_run.console_output"), expanded=False):
                col1, col2 = st.columns([6,1])
                with col2:
                    if st.button(t("button.clear_console")):
                        ss.console_output = []
                        st.rerun()
